...
class BroadcastLinkMessageUseCaseTest : UseCaseTest() {
    ...
    private fun mockkDeserializeToMap(
        inputValue: String = mockk(),
        returnValue: Map<String, *>,
    ) {
        every { jsonConverter.toMap(inputValue) } returns returnValue
    }

    private fun mockkFilterLinkMessage(
        inputValue: Map<String, *> = mockk(),
        returnValue: Map<String, *>?,
    ) {
    ...
    @Test
    fun `test useCase broadcasts the message correctly`() =
        runTest {
    ...
    @Test
    fun `test useCase does not broadcast a map on filtered out map`() =
        runTest {
            val json = randomString
            val convertedMap: Map<String, *> = mockk()
